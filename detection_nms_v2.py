...
from lib1.session_worker import SessionWorker
from lib1.load_graph_nms_v2 import LoadFrozenGraph
from lib1.load_label_map import LoadLabelMap
from lib1.mpvariable import MPVariable
from lib1.mpvisualizeworker import MPVisualizeWorker, visualization
from lib1.mpio import start_sender
import yaml
import time
import cv2
...
from lib1.pyimagesearch.main1 import Panaroma
...
class NMSV2():
    ...
    def video_feed():
        ...
        # type (mime type)
        return Response(NMSV2.generate(),
    ...
    def start1(self):
        ...
        """ """ """ """ """ """ """ """ """ """ """
        if SRC_FROM == CAMERA:
            #from lib1.webcam import WebcamVideoStream as VideoReader
            pass
        elif SRC_FROM == MOVIE:
            ...
            while MPVariable.running.value:
                ...
                if gpu_worker.is_sess_empty(): # must need for speed
                        ...
                        if SRC_FROM == IMAGE:
                            ...
                            #time.sleep(3.0)
                            left = imutils.resize(left, width=900)
                            right = imutils.resize(right, width=900)
    #stitcher = Stitcher1()
    
    
                            stitched = stitcher.image_stitch([left,right])
                            print("[INFO] cropping...")
                ...
                image = visualization(category_index, image, boxes, scores, classes, DEBUG_MODE, VIS_TEXT, FPS_INTERVAL,
                                      fontFace=fontFace, fontScale=1, fontThickness=2)
...
if __name__ == "__main__":
        ...
        t = threading.Thread(target=NMSV2.start1, args=(
            args["frame_count"],))
        t.daemon = True
        t.start()
        app.run(host='127.0.0.1', port=5000)
